...
export async function makeLogMachine(bot) {
    await jumpForDuration(bot, 5000);
    await walkNorth(bot, 30);
    await acquireItems(bot);
    await setOrigin(bot);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -1]);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -2]);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -3]);
    await placeOnBlock(bot, "smooth_stone", [-1, -1, -2]);
    await walkTo(bot, 1, 0, -4);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -4]);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -5]);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -6]);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -7]);
    await walkTo(bot, 1, 0, -8);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -8]);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -9]);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -10]);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -11]);
    await placeOnBlock(bot, "smooth_stone", [-1, -1, -12]);
    await walkTo(bot, 1, 0, -12);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -12]);
    await placeOnBlock(bot, "smooth_stone", [0, -1, -13]);
    await placeOnBlock(bot, "smooth_stone", [-1, -1, -13]);
    await placeOnBlock(bot, "smooth_stone", [-2, -1, -13]);
    await walkTo(bot, -3, 0, -14);
    await placeOnBlock(bot, "smooth_stone", [-3, -1, -13]);
    await placeOnBlock(bot, "smooth_stone", [-4, -1, -13]);
    await placeOnBlock(bot, "smooth_stone", [-5, -1, -13]);
    await placeOnBlock(bot, "smooth_stone", [-6, -1, -13]);
    await walkTo(bot, -7, 0, -14);
    await placeOnBlock(bot, "smooth_stone", [-7, -1, -13]);
    await placeOnBlock(bot, "smooth_stone", [-8, -1, -13]);
    await placeOnBlock(bot, "smooth_stone", [-9, -1, -13]);
    await placeOnBlock(bot, "smooth_stone", [-10, -1, -13]);
    await placeOnBlock(bot, "smooth_stone", [-10, -1, -12]);
    await walkTo(bot, -13, 0, -12);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -13]);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -12]);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -11]);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -10]);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -9]);
    await walkTo(bot, -12, 0, -8);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -8]);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -7]);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -6]);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -5]);
    await walkTo(bot, -12, 0, -4);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -4]);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -3]);
    await placeOnBlock(bot, "smooth_stone", [-10, -1, -2]);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -2]);
    await placeOnBlock(bot, "smooth_stone", [-11, -1, -1]);
    await placeOnBlock(bot, "smooth_stone", [-10, -1, -1]);
    await walkTo(bot, -12, 0, 0);
    await placeOnBlock(bot, "smooth_stone", [-9, -1, -1]);
    await placeOnBlock(bot, "smooth_stone", [-8, -1, -1]);
    await walkTo(bot, -7, 0, 1);
    await placeOnBlock(bot, "smooth_stone", [-7, -1, -1]);
    await placeOnBlock(bot, "smooth_stone", [-6, -1, -1]);
    await placeOnBlock(bot, "smooth_stone", [-5, -1, -1]);
    await placeOnBlock(bot, "smooth_stone", [-4, -1, -1]);
    await walkTo(bot, -3, 0, 1);
    await placeOnBlock(bot, "smooth_stone", [-3, -1, -1]);
    await placeOnBlock(bot, "smooth_stone", [-2, -1, -1]);
    await placeOnBlock(bot, "smooth_stone", [-1, -1, -1]);
    await walkTo(bot, 0, 0, 0);
    await pourWaterInHole(bot, -1, 1, -2);
    await walkTo(bot, 0, 0, -14);
    await pourWaterInHole(bot, -1, 1, -12);
    await walkTo(bot, -12, 0, -14);
    await pourWaterInHole(bot, -10, 1, -12);
    await walkTo(bot, -12, 0, 0);
    await pourWaterInHole(bot, -10, 1, -2);
    await walkTo(bot, -6, 0, -7);
    await placeOnBlock(bot, "grass_block", [-5, -1, -7]);
    await placeOnBlock(bot, "grass_block", [-5, 0, -7]);
    await placeOnBlock(bot, "grass_block", [-5, 1, -7]);
    await placeOnBlock(bot, "smooth_stone", [-5, 2, -7]);
    await placeOnBlock(bot, "grass_block", [-5, 3, -7]);
    await placeOnBlock(bot, "grass_block", [-5, 4, -7]);
    await placeOnBlock(bot, "obsidian", [-5, 5, -7], new Vec3(-1, 0, 0));
    await placeOnBlock(bot, "grass_block", [-6, 5, -7], new Vec3(-1, 0, 0));
    await placeOnBlock(bot, "grass_block", [-7, 5, -7], new Vec3(-1, 0, 0));
    await placeOnBlock(bot, "obsidian", [-8, 5, -7]);
    const blocks_to_dig_obsidian = [
        [-5, 0, -7], [-5, 1, -7], [-5, 2, -7],
        [-5, 4, -7], [-5, 5, -7], [-7, 5, -7],[-8, 5, -7]
    ];
    await dig(bot, blocks_to_dig_obsidian);
    return
